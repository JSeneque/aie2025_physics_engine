...
Box::Box(glm::vec2 position, glm::vec2 velocity, float mass, glm::vec2 extents, glm::vec4 colour, glm::vec2 localX, glm::vec2 localY):
...
glm::vec2 Box::getExtents() const
{
	return m_extents;
}

glm::vec2 Box::getLocalX() const
{
	return m_localX;
}
	...
	float& pen, glm::vec2& edgeNormal)
{
	...
	// if we lie entirely to one side of the box along one axis, we've found a
	//separating
